...
- **Python Components (`python_connector/`)**: Flexible exchange adapters, data science integrations, and rapid prototyping of connectors.
- **Legacy Components (`archive/legacy/`)**: Historical Go-based monolithic implementation archived for reference. It is NOT part of the active workspace or build process.

### 1.1 Development Standards

- **Go**: Version 1.24+, managed via `go.work` workspace.
- **Python**: Version 3.14+, managed via `pyproject.toml` and `uv`.
- **Inter-service Communication**: Strictly via gRPC defined in `api/proto/`.
- **Model Consistency (Audit Goal)**: All exchange connectors (Go & Python) MUST map exchange-specific data to the standardized Protobuf models consistently. No "leakage" of raw exchange data types into core logic.

### 1.2 Code Quality Gate (REQ-MAINT-*)

- **Static Audit (REQ-MAINT-001)**: `cd market_maker && make audit` MUST be clean.
- **Naming Conventions (REQ-MAINT-002)**:
  - Protobuf fields MUST be `snake_case` (e.g., `client_order_id`).
  - Generated Go protobuf fields (e.g., `OrderId`) MUST NOT be renamed manually.
  - Hand-written Go code MUST use idiomatic acronyms for locals/params (e.g., `orderID`, `clientOrderID`) to satisfy `staticcheck` ST1003.
- **Vulnerability Scanning (REQ-MAINT-003)**:
  - `govulncheck` SHOULD run as part of audit.
  - If the vulnerability DB fetch is blocked locally (e.g., HTTP 403 to `vuln.go.dev`), the local audit MAY warn-but-not-fail.
  - CI MUST run `govulncheck` in an environment with access to the vuln DB and fail on findings.

### 1.3 Delivery Process (REQ-PROC-*)

- **Specs-First (REQ-PROC-001)**: Non-trivial changes MUST start with a written spec/update in `docs/specs/` before code.
- **TDD (REQ-PROC-002)**: Logic changes MUST add/extend tests (RED), then implement (GREEN), then refactor.
- **Audit Hook (REQ-PROC-003)**: Every change touching Go code MUST be validated with `cd market_maker && make audit` prior to merge.
- **Atomic Commits (REQ-PROC-004)**: Keep one logical change per commit and avoid mixing generated artifacts with manual edits.
- **Clean Worktree (REQ-PROC-005)**: Do not commit build/test artifacts (pyc, db-shm/wal, binaries); ensure `.gitignore` covers them and untrack if they appear.

### 1.4 Data Model Invariants (REQ-DM-*)
- **REQ-DM-001**: Protobuf fields remain `snake_case`; generated Go/Python structs are not manually renamed.
- **REQ-DM-002**: Idempotency hinges on `client_order_id` for order placement; connectors MUST honor duplicate detection semantics.
- **REQ-DM-003**: Timestamps use protobuf `Timestamp` or millisecond int64 consistently per proto definitions; no mixed units.
- **REQ-DM-004**: Decimal values use `google.type.Decimal` and must round-trip through storage (SQLite/DBOS) without precision loss.

### 1.5 Workflow & E2E Expectations (REQ-WF-*)
- **REQ-WF-001**: Durable workflows (DBOS) must wrap side effects in steps (`RunAsStep`) to ensure exactly-once behavior on replay.
- **REQ-WF-002**: Simple engine persistence must restore `State` (slots, last_price) identically after crash/restart.
- **REQ-WF-003**: E2E tests MUST cover crash recovery, risk protection triggers, gRPC stack health/streams, and DBOS replay; gaps are filled via TDD when found.
- **REQ-WF-004**: `make audit` + `make test` are the acceptance gate for workflow changes; failing tests imply spec/doc updates before code changes.

### 1.6 Grid Trading Invariants (REQ-GRID-*)
- **REQ-GRID-001**: Slot lifecycle is ordered: FREE → LOCKED → (FILLED | CANCELED) → FREE; no orphaned LOCKED slots after recovery.
- **REQ-GRID-002**: Risk trigger disables opening BUYs and cancels active BUY orders; SELL behavior follows the configured strategy mode (neutral vs directional).
- **REQ-GRID-003**: Idempotency: `client_order_id` must be set for placed orders; subsequent placements with the same `client_order_id` must not duplicate orders.
- **REQ-GRID-004**: Persistence: `State` restore rehydrates slot/order/client maps so that `GetOrder` and order updates route to the correct slot after crash.
- **REQ-GRID-005**: Pricing: all decisions use consistent decimals/rounding per proto precision (price/qty decimals) to avoid drift across grid levels.

### 1.7 Testing & CI (REQ-TEST-*)
- **REQ-TEST-001**: Offline suites MUST run in CI: `go test -race ./internal/...`, `go test -race ./pkg/...`, and targeted E2E (`TestE2E_CrashRecovery|RiskProtection|TradingFlow`).
- **REQ-TEST-002**: Integration E2E (startup, loopback, health, DBOS, arbitrage) MUST be gated (env/tag) and have a documented run condition; CI runs them when dependencies are available.
- **REQ-TEST-003**: All workflow-affecting changes update specs first, add/extend tests (RED), then implement (GREEN), then refactor with gates green.
- **REQ-TEST-004**: New grid logic must add/extend offline E2E or unit tests covering: idempotent `client_order_id`, risk-trigger cancellation, and crash restore routing.
- **REQ-TEST-005**: Record latest grid test outcomes in `docs/specs/grid_workflow_audit_spec.md` and keep `plan.md` Phase 26 progress updated after each TDD cycle.

### 1.8 Funding Data Model (REQ-FUND-*)
- **REQ-FUND-001**: `FundingRate` / `FundingUpdate` timestamps (`timestamp`, `next_funding_time`) are Unix epoch milliseconds (UTC); `0` means “not applicable/unknown” (e.g., spot).
- **REQ-FUND-002**: `predicted_rate` is optional; unset if the venue does not provide it; meaning: expected funding for the next interval, same units as `rate`.
- **REQ-FUND-003**: Spot connectors return `rate=0`, `next_funding_time=0`, with a valid `timestamp` set to connector receipt time.
- **REQ-FUND-004**: Protos MUST be regenerated whenever FundingRate/FundingUpdate semantics change.
- **REQ-FUND-005**: Funding monitoring MUST be symbol-configurable (no hardcoded symbols) and expose staleness detection (TTL) per exchange/symbol.

### 1.9 Arbitrage Workflows (REQ-ARB-*)
- **REQ-ARB-001**: Funding signal uses spread (short – long) with correct interval; APR uses actual interval (or configured fallback), not hardcoded 8h.
- **REQ-ARB-002**: Each external order in a workflow must carry deterministic `client_order_id` to be idempotent across retries/replays.
- **REQ-ARB-003**: Only one entry/exit workflow per symbol may be in-flight; duplicate triggers must be deduped.
- **REQ-ARB-004**: Partial fills must size opposite legs and compensations from executed quantities, not requested quantities.
- **REQ-ARB-005**: Funding streams must run for both legs (long/short) before emitting spread decisions; missing leg data MUST block entry.
- **REQ-ARB-006**: Spread/APR calculation must be derived from per-interval rates with explicit interval hours; helper functions must be covered by unit tests.
- **REQ-ARB-007**: Directionality for spot–perp funding arb: positive funding → long spot / short perp (collect funding); negative funding → borrow/short spot and long perp (paying funding on spot borrow is acceptable per venue limits); strategy must gate on borrow/position limits.
- **REQ-ARB-008**: `PlaceOrderRequest` MUST carry `use_margin` for spot legs when shorting or borrowing; engines must set it deterministically per leg, and connectors must honor it.
- **REQ-ARB-009**: Spot connectors that support margin (starting with Binance) MUST route margin orders when `use_margin=true`, preserve idempotent `client_order_id`, and map margin-specific errors to standard `apperrors`.
- **REQ-ARB-010**: Tests MUST cover positive/negative funding scenarios with and without margin availability; margin rejections must surface as deterministic, mapped errors and block entry.
- **REQ-ARB-011**: Borrow-limit and margin-disabled responses MUST map to a stable, retry-safe error category that prevents re-entry until conditions change.
- **REQ-ARB-012**: Idempotency for margin orders MUST be identical to non-margin orders; duplicate `client_order_id` should return mapped “already exists” without placing new orders.
- **REQ-ARB-013**: Phase 28 changes MUST be gated by `cd market_maker && make audit`, `cd market_maker && make test`, and Python connector unit tests; failures require spec updates before code changes.
- **REQ-ARB-014**: Margin-specific errors (e.g., margin disabled, borrow cap exceeded, insufficient collateral, duplicate client_order_id) MUST map to deterministic `apperrors` codes consistent across Go/Python connectors.
- **REQ-ARB-015**: Margin routing MUST fall back or fail fast based on venue capability; no silent downgrade to spot without margin when `use_margin=true`.
- **REQ-ARB-016**: `use_margin` defaults to `false` to preserve backward compatibility; enabling requires explicit planner intent.
- **REQ-ARB-017**: Proto changes (`use_margin`) MUST be regenerated via `buf` for both Go and Python; generated code must not be manually edited.
- **REQ-ARB-018**: Workflow replay/retry MUST preserve the `use_margin` flag end-to-end; DBOS/simple engines must not drop or change the flag across steps.
- **REQ-ARB-019**: All platforms (Go/Python) MUST run full test gates post-implementation: `make audit`, `make test` in Go tree, and pytest suite in `python_connector` (ensure pytest available in CI/runtime).
