...
class BinanceConnector(exchange_pb2_grpc.ExchangeServiceServicer):
    ...
    async def GetType(self, request, context):
        ...
        # TODO: Detect real UM status from account fetch
    ...
    async def GetSymbolInfo(self, request, context):
        ...
        # CCXT precision is decimal places, but proto wants tick_size (increment)
        # Some exchanges provide the increment directly in 'info' or filters
        tick_size = "0"
        step_size = "0"
    ...
    async def BatchPlaceOrders(self, request, context):
        ...
        for res in results:
            if isinstance(res, Exception):
                logger.error(f"Batch order component failure: {res}")
                all_success = False
            else:
    ...
    async def _get_account_impl(self) -> models_pb2.Account:
        balance = await self.exchange.fetch_balance()
        # Assumes USDT based futures account for simplicity
        total_wallet = balance.get("total", {}).get("USDT", 0)
        available = balance.get("free", {}).get("USDT", 0)

        positions_resp = await self._get_positions_impl(None)
    ...
    async def GetAccount(self, request, context):
        return await self._get_account_impl()
    ...
    async def GetPositions(self, request, context):
        return await self._get_positions_impl(request.symbol)
    ...
    def _to_decimal(self, value: Union[str, float, int, None]) -> decimal_pb2.Decimal:
        ...
        if isinstance(value, float):
            # Use format to avoid scientific notation and binary artifacts
            s = "{:.12f}".format(value).rstrip("0").rstrip(".")
            if s == "" or s == "-":
                s = "0"
